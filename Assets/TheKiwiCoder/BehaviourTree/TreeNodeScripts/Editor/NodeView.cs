...
namespace TheKiwiCoder
{
    public class NodeView : UnityEditor.Experimental.GraphView.Node
    {
        ...
        public NodeView(SerializedBehaviourTree tree, Node node) : base(
            AssetDatabase.GetAssetPath(BehaviourTreeSettings.GetOrCreateSettings().nodeXml))
        {
            this.serializer = tree;
            if (node != null)
            {
                if (node.NodeTypeName.Equals(node.GetName()))
                {
                    this.title ="<b>"+ node.GetName()+"</b>";
                }
                else
                {
                    this.title ="<b>"+  node.NodeTypeName + "</b>\n" + node.GetName();
                }
